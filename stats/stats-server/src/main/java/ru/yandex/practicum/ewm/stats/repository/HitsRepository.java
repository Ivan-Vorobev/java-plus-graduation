package ru.yandex.practicum.ewm.stats.repository;
...
public interface HitsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query("SELECT new ru.yandex.practicum.dto.HitsStatDTO(h.app, h.uri, COUNT(h)) " +
           "FROM Hit h " +
    ...
    @Query("SELECT new ru.yandex.practicum.dto.HitsStatDTO(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
           "FROM Hit h " +
           "WHERE h.timestamp BETWEEN :start AND :end " +
           "GROUP BY h.app, h.uri " +
